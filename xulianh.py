...
def chia_o_vuong(warped_img1):
    o_vuong = []
    width = warped_img1.shape[0] // 9
    ...
    for j in range(9):
        for i in range(9):
            ...
            o_vuong.append(warped_img1[p1[1]:p2[1], p1[0]:p2[0]])
...
# Hàm tìm kiếm contours, vẽ hình chữ nhật, cắt ảnh và resize
def process_images(images):
    processed_images = []
    for img in images:
        if isinstance(img, int):  # Kiểm tra nếu là số nguyên (ví dụ 0)
...
# Hàm nhận diện chữ số (đã cho)
def nhandien_chuso(o_vuong_daxuli1, model):
    o_vuong_daxuli = process_images(o_vuong_daxuli1)
    ...
    o_vuong_danhandang=[]
    # ảnh trắng
    anh_trang = np.zeros_like(cv2.resize(o_vuong_daxuli[0], (32, 32)))
    ...
    for i in range(len(o_vuong_daxuli)):
        if type(o_vuong_daxuli[i]) == int:
            ...
            anh = cv2.resize(o_vuong_daxuli[i], (32, 32))
            o_vuong_dadinhdang.append(anh)

    o_vuong_dadinhdang = np.array(o_vuong_dadinhdang)
    for i in range(len(o_vuong_dadinhdang)):
        anh1 = o_vuong_dadinhdang[i]
        anh1 = np.expand_dims(anh1, axis=0)
        anh1 = np.array(anh1)
        anh1 = np.expand_dims(anh1, axis=-1)
        anh1 = np.repeat(anh1, 3, axis=-1)
        nhan_dien = model.predict(anh1)
        o_vuong_danhandang.append(nhan_dien)
    all_preds = list(map(np.argmax, o_vuong_danhandang))
